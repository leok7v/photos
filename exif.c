...
#define MAX_IFD_TAGS 32
#define MAX_IFD_NESTING_LEVEL 15
#define UNEXPECTED(e) (e)
#define EXIFERR_CC
#define E_WARNING -1
#define E_NOTICE  -2
...
static int php_strnlen(const char* s, size_t n) {
    int k = 0;
    while (k < n && *s++ != 0) { k++; }
    return k;
}
...
/* Internal */
#define TAG_NONE               			((unsigned short)-1) /* note that -1 <> 0xFFFF */
#define TAG_COMPUTED_VALUE     			((unsigned short)-2)
#define TAG_END_OF_LIST                 0xFFFDU
...
static char *exif_get_sectionlist(int sectionlist)
{
    ...
    for(i=0; i<SECTION_COUNT; i++) {
        ml += (int)strlen(exif_get_sectionname(i)) + 2;
    }
    sections = malloc(ml);
    sections[0] = '\0';
    len = 0;
...
{
    (void)docref; (void)ImageInfo; (void)type; (void)format;
#if 0
    va_list args;
...
static __inline bool ptr_offset_overflows(char *ptr, size_t offset) {
...
static inline void exif_offset_info_init(
    ...
    assert(!ptr_offset_overflows(valid_start, valid_length));
#ifdef __SANITIZE_ADDRESS__
    assert(!__asan_region_is_poisoned(valid_start, valid_length));
#endif
...
static int exif_file_sections_add(image_info_type *ImageInfo, int type, size_t size, uchar *data)
{
    ...
    if (count == ImageInfo->file.alloc_count) {
        ...
        ImageInfo->file.list = realloc(
            ImageInfo->file.list, new_alloc_count * sizeof(file_section));
...
//		EXIF_ERRLOG_FSREALLOC(ImageInfo)
    ...
    tmp = realloc(ImageInfo->file.list[section_index].data, size);
...
static bool exif_file_sections_free(image_info_type *ImageInfo)
{
    ...
    if (ImageInfo->file.count) {
        for (i=0; i<ImageInfo->file.count; i++) {
            free(ImageInfo->file.list[i].data);
        }
    }
    free(ImageInfo->file.list);
...
static image_info_data *exif_alloc_image_info_data(image_info_list *info_list) {
    if (info_list->count == info_list->alloc_count) {
        ...
        info_list->list = realloc(
            info_list->list, new_alloc_count * sizeof(image_info_data));
...
static void exif_iif_add_value(image_info_type *image_info, int section_index, char *name, int tag, int format, int length, void* value, size_t value_len, int motorola_intel)
{
    ...
    info_data->tag    = (unsigned short)tag;
    info_data->format = (unsigned short)format;
    info_data->length = length;
    info_data->name   = strdup(name);
...
//   			exif_error_docref("exif_iif_add_value" EXIFERR_CC, image_info, E_WARNING, "length > value_len: %d > %zu", length, value_len);
        ...
        case TAG_FMT_DOUBLE:
            ...
            if (length>1) {
                info_value->list = malloc(length * sizeof(image_info_value));
            } else {
                info_value = &info_data->value;
            }
...
static void exif_iif_add_str(image_info_type *image_info, int section_index, char *name, char *value)
{
    if (value) {
        ...
        info_data->length = 1;
        info_data->name   = strdup(name);
        info_data->value.s = strdup(value);
        image_info->sections_found |= 1<<section_index;
    }
}
/* }}} */

static void vspprintf(char** p, int n, const char* format, va_list vl) {
    *p = malloc(n + 1);
    vsnprintf(*p, n, format, vl);
}

static void spprintf(char** p, int n, const char* format, ...) {
    va_list vl;
    va_start(vl, format);
    vspprintf(p, n, format, vl);
    va_end(vl);
}
...
static void exif_iif_add_buffer(image_info_type *image_info, int section_index, char *name, int length, char *value)
{
    if (value) {
        ...
        info_data->length = length;
        info_data->name   = strdup(name);
        info_data->value.s = malloc(length);
...
static void exif_iif_free(image_info_type *image_info, int section_index) {
    ...
    free(image_info->info_list[section_index].list);
}
/* }}} */
#if 0
...
static void* exif_ifd_make_value(image_info_data *info_data, int motorola_intel) {
    ...
    value_ptr = malloc(max(byte_count, 4));
    ...
    if (info_data->format == TAG_FMT_UNDEFINED || info_data->format == TAG_FMT_STRING
      || (byte_count>1 && (info_data->format == TAG_FMT_BYTE || info_data->format == TAG_FMT_SBYTE))
        ...
        *value_ptr = (char)info_data->value.u;
        return value_ptr;
    } else if (info_data->format == TAG_FMT_SBYTE) {
        *value_ptr = (char)info_data->value.i;
        return value_ptr;
    } else {
        data_ptr = value_ptr;
...
static void exif_thumbnail_build(image_info_type *ImageInfo) {
    ...
    switch(ImageInfo->Thumbnail.filetype) {
        ...
        case IMAGE_FILETYPE_TIFF_MM:
            ...
            new_move = new_size;
            new_data = realloc(ImageInfo->Thumbnail.data, ImageInfo->Thumbnail.size + new_size);
...
static int exif_process_string(char **result, char *value, size_t byte_count) {
     ...
     * char.
     * strdup would sometimes allocate more memory and does not return length
     */
    ...
    *pszEncoding = NULL;
    ...
    if (ByteCount>=8) {
        ...
        if (!memcmp(szValuePtr, "UNICODE\0", 8)) {
            *pszEncoding = strdup((const char*)szValuePtr);
...
static bool exif_process_IFD_TAG_impl(image_info_type *ImageInfo, char *dir_entry, const exif_offset_info *info, size_t displacement, int section_index, int ReadNextIFD, tag_table_type tag_table)
{
    ...
    char *value_ptr, cbuf[32], *outside=NULL;
...
//              exif_error_docref("exif_read_data#error_ifd" EXIFERR_CC, ImageInfo, E_WARNING, "Process tag(x%04X=%s): Illegal pointer offset(x%04X + x%04X = x%04X > x%04X)", tag, exif_get_tagname_debug(tag, tag_table), offset_val, byte_count, offset_val+byte_count, ImageInfo->FileSize);
            ...
            fseek(ImageInfo->infile, (long)displacement + (long)offset_val, SEEK_SET);
...
//              exif_error_docref(NULL EXIFERR_CC, ImageInfo, E_WARNING, "Wrong file pointer: 0x%08X != 0x%08X", fgot, displacement+offset_val);
            ...
            fgot = fread(value_ptr, byte_count, 1, ImageInfo->infile);
            fseek(ImageInfo->infile, (int)fpos, SEEK_SET);
            if (fgot != byte_count) {
                free(outside);
    ...
    if (section_index==SECTION_THUMBNAIL) {
        ...
        switch(tag) {
            case TAG_COPYRIGHT:
                ...
                if (byte_count>1 && (length=php_strnlen(value_ptr, byte_count)) > 0) {
                    if (length<byte_count-1) {
                        ...
                        free(ImageInfo->CopyrightEditor);
                        free(ImageInfo->Copyright);
                        ImageInfo->CopyrightPhotographer  = strdup(value_ptr);
            ...
            case TAG_USERCOMMENT:
                free(ImageInfo->UserComment);
                ImageInfo->UserComment = NULL;
                free(ImageInfo->UserCommentEncoding);
...
//              ImageInfo->UserCommentLength = exif_process_user_comment(ImageInfo, &(ImageInfo->UserComment), &(ImageInfo->UserCommentEncoding), value_ptr, byte_count);
            ...
            case TAG_XP_SUBJECT:
                tmp_xp = (xp_field_type*)realloc(ImageInfo->xp_fields.list, (ImageInfo->xp_fields.count+1) * sizeof(xp_field_type));
...
//              exif_process_unicode(ImageInfo, &(ImageInfo->xp_fields.list[ImageInfo->xp_fields.count-1]), tag, value_ptr, byte_count);
            ...
            case TAG_MAX_APERTURE:
                ...
                if (ImageInfo->ApertureFNumber == 0) {
                    REQUIRE_NON_EMPTY();
                    ImageInfo->ApertureFNumber = expf((float)exif_convert_any_format(value_ptr, format, ImageInfo->motorola_intel)*logf(2.0f)*0.5f);
                }
                break;
            ...
            case TAG_SHUTTERSPEED:
                ...
                if (ImageInfo->ExposureTime == 0) {
                    REQUIRE_NON_EMPTY();
                    ImageInfo->ExposureTime = expf((float)-exif_convert_any_format(value_ptr, format, ImageInfo->motorola_intel)*logf(2.0f));
                }
                break;
            ...
            case TAG_MAKER_NOTE:
                if (!exif_process_IFD_in_MAKERNOTE(ImageInfo, value_ptr, (int)byte_count, info, displacement)) {
                    free(outside);
                    return false;
                }
                break;
...
static bool exif_scan_JPEG_header(image_info_type *ImageInfo)
{
    ...
    while (true) {
        ...
        if ((lh = fgetc(ImageInfo->infile)) == (unsigned int)EOF) {
            EXIF_ERRLOG_CORRUPT(ImageInfo)
            return false;
        }
        if ((ll = fgetc(ImageInfo->infile)) == (unsigned int)EOF) {
            EXIF_ERRLOG_CORRUPT(ImageInfo)
            return false;
        }
        ...
        got = (long)fread((char*)(Data+2), itemlen-2, 1, ImageInfo->infile); /* Read the whole section. */
        if (got != itemlen-2) {
        ...
        switch(marker) {
            case M_SOS:   /* stop before hitting compressed data  */
                ...
                if (ImageInfo->read_all) {
                    ...
                    Data = ImageInfo->file.list[sn].data;
                    got = (long)fread((char*)Data, size, 1, ImageInfo->infile);
                    if (got != size) {
                        EXIF_ERRLOG_FILEEOF(ImageInfo)
                        return false;
                    }
            ...
            case M_SOF15:
                ...
                ImageInfo->Width  = (int)sof_info.width;
                ImageInfo->Height = (int)sof_info.height;
...
static bool exif_process_IFD_in_TIFF_impl(image_info_type *ImageInfo, size_t dir_offset, int section_index)
{
    ...
    if (ImageInfo->FileSize >= 2 && ImageInfo->FileSize - 2 >= dir_offset) {
...
#endif
        fseek(ImageInfo->infile, (long)dir_offset, SEEK_SET); /* we do not know the order of sections */
        if (UNEXPECTED(fread((char*)ImageInfo->file.list[sn].data, 2, 1, ImageInfo->infile) != 2)) {
            return false;
        }
        ...
        if (ImageInfo->FileSize >= dir_size && ImageInfo->FileSize - dir_size >= dir_offset) {
            ...
            if (UNEXPECTED(fread((char*)(ImageInfo->file.list[sn].data+2), dir_size-2, 1, ImageInfo->infile) != dir_size - 2)) {
                return false;
            }
...
//                  exif_error_docref(NULL EXIFERR_CC, ImageInfo, E_NOTICE, "Read from TIFF: tag(0x%04X,%12s): Illegal format code 0x%04X, switching to BYTE", entry_tag, exif_get_tagname_debug(entry_tag, tag_table), entry_type);
                ...
                if (entry_length <= 4) {
                    ...
                    switch(entry_tag) {
                        case TAG_IMAGEWIDTH:
                        case TAG_COMP_IMAGE_WIDTH:
                            ImageInfo->Width  = (int)entry_value;
                            break;
                        case TAG_IMAGEHEIGHT:
                        case TAG_COMP_IMAGE_HEIGHT:
                            ImageInfo->Height = (int)entry_value;
                            break;
...
#endif
                    fread((char*)(ImageInfo->file.list[sn].data+dir_size), ifd_size-dir_size, 1, ImageInfo->infile);
#ifdef EXIF_DEBUG
...
#endif
                                if (!ImageInfo->Thumbnail.data) {
                                    ImageInfo->Thumbnail.data = malloc(ImageInfo->Thumbnail.size);
                                    fseek(ImageInfo->infile, (long)ImageInfo->Thumbnail.offset, SEEK_SET);
                                    fgot = fread(ImageInfo->Thumbnail.data, ImageInfo->Thumbnail.size, 1, ImageInfo->infile);
                                    if (fgot != ImageInfo->Thumbnail.size) {
                                        EXIF_ERRLOG_THUMBEOF(ImageInfo)
                                        free(ImageInfo->Thumbnail.data);
...
static bool exif_scan_FILE_header(image_info_type *ImageInfo)
{
    ...
    if (ImageInfo->FileSize >= 2) {
        ...
        if (fread((char*)file_header, 2, 1, ImageInfo->infile) != 2) {
            return false;
        }
        if ((file_header[0]==0xff) && (file_header[1]==M_SOI)) {
            ...
            if (fread((char*)(file_header+2), 6, 1, ImageInfo->infile) != 6) {
                return false;
            }
...
static bool exif_discard_imageinfo(image_info_type *ImageInfo)
{
    ...
    free(ImageInfo->CopyrightEditor);
    free(ImageInfo->Thumbnail.data);
    free(ImageInfo->encode_unicode);
    free(ImageInfo->decode_unicode_be);
    free(ImageInfo->decode_unicode_le);
    free(ImageInfo->encode_jis);
    free(ImageInfo->decode_jis_be);
    free(ImageInfo->decode_jis_le);
    free(ImageInfo->make);
    free(ImageInfo->model);
    for (i=0; i<ImageInfo->xp_fields.count; i++) {
        free(ImageInfo->xp_fields.list[i].value);
    }
    free(ImageInfo->xp_fields.list);
    for (i=0; i<SECTION_COUNT; i++) {
...
/* {{{ exif_read_from_impl */
static bool exif_read_from_impl(image_info_type *ImageInfo, FILE* stream, int read_thumbnail, int read_all)
{
    bool ret;
    struct stat_t st = {0};
    ...
    if (php_stream_is(ImageInfo->infile, PHP_STREAM_IS_STDIO)) {
        ...
        if (!ImageInfo->FileSize) {
            ...
            ImageInfo->FileSize = ftell(ImageInfo->infile);
            fseek(ImageInfo->infile, 0, SEEK_SET);
        }
    }
    ...
    ImageInfo->encode_unicode		= strdup(EXIF_G(encode_unicode));
    ImageInfo->decode_unicode_be	= strdup(EXIF_G(decode_unicode_be));
    ImageInfo->decode_unicode_le	= strdup(EXIF_G(decode_unicode_le));
    ImageInfo->encode_jis			= strdup(EXIF_G(encode_jis));
    ImageInfo->decode_jis_be	 	= strdup(EXIF_G(decode_jis_be));
    ImageInfo->decode_jis_le		= strdup(EXIF_G(decode_jis_le));
...
static bool exif_read_from_stream(image_info_type *ImageInfo, FILE* stream, int read_thumbnail, int read_all)
{
    bool ret;
    off_t old_pos = ftell(stream);
    ...
    if (old_pos) {
        fseek(stream, (long)old_pos, SEEK_SET);
    }
